...
public class ArcadeDrive1Joy extends Command {
    @SuppressWarnings({ "PMD.UnusedPrivateField", "PMD.SingularField" })
    private final Drivetrain drivetrain;
    private final XBController controller;

    /**
     * Creates the new ArcadeDrive1Joy.
     * 
     * @param drivetrain The drivetrain of the robot.
     * @param controller The Xbox controller used to operate the robot.
     */
    public ArcadeDrive1Joy(Drivetrain drivetrain, XBController controller) {
    ...
    @Override
    public void execute() {
        /* Initialize forward/back movement speed based on joystick Y axis with deadband 0.1 */
        double moveSpeed = controller.getLeftJoyY();
        if (Math.abs(moveSpeed) < 0.1)
            moveSpeed = 0;

        /* Initialize rotation speed based on joystick X axis with deadband 0.1 */
        double rotSpeed = controller.getLeftJoyX();
